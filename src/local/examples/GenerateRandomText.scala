...
object GenerateRandomText {
  ...
  def main(args: Array[String]) {
    val outputPath = new File("/Users/xulijie/Documents/data/RandomText/randomText-10MB.txt")
    ...
    while (outputPath.length() < 10 * 1024 * 1024) {
      var index = Math.abs(ranGen.nextGaussian() * 1000) % 1000
      writer.write(words.apply(index.toInt))
      writer.write(" ")
      index = Math.abs(ranGen.nextGaussian()) % 1000
      writer.write(words.apply(index.toInt))
      writer.write("\n")
    }
    writer.close()
